/* Solis to MQTT Requester V310523-1 Copyright @ Pit Hermann 01.04.2023 */

/* You use this software on your own risk !!!
 * The software is provided as it is.
 * 
 * This software depends on the device type register implemented by Solis 2020/02/20
 * (storage-)inverter that do not contain this register or if the content of the register 
 * is not equal to 0x00 or 0x20 this software doesn't work.
 * Find within the typedetect folder a more or less simple way to check if this software is useable
 * with your (storage-)inverter.
 * 
 * needed libraries:
...
uint8_t FirstRun = 1;
uint8_t DeviceType = 99; // 99 = auto ; 0 = std inverter ; 32 = storage inverter
...
void onTelnetConnect(String ip) {
  ...
  telnet.println("\nWelcome @ Solis to MQTT Requester V310523-1");
...
void S_E_msgs(){
  ...
  delay(100);
  switch (DeviceType){
    case  10  : inv_date = "Actually not";
                inv_sn = "supported device,";
                inv_time = "maybe in future";
                telnet.println("Actually not supported device, maybe in future. I'm looking for needed data");
                break;
    case  55  : inv_date = "No device type";
                inv_sn = "avaiable,";
                inv_time = "old firmware";
                telnet.println("No device type avaiable, old firmware , doesn't support this request.");
                break; 
...
void loop() {
  ...
  if ((FirstRun == 1) && (DeviceType == 0)){
    sendDiscoveryMsgs();
    FirstRun = 0;
    delay(100);
  }
  else if ((FirstRun == 1) && (DeviceType == 32)){
    sendDiscoveryMsgsH();
    FirstRun = 0;
    delay(100);
  }
